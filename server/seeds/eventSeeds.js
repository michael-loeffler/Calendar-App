const seedEvents = [
  {
    title: 'Team meeting',
    start: new Date(2023, 4, 2, 14, 0),
    end: new Date(2023, 4, 2, 16, 0),
  },
  {
    title: 'Lunch with John',
    start: new Date(2023, 4, 3, 12, 0),
    end: new Date(2023, 4, 3, 13, 0),
  },
  {
    title: 'Dentist appointment',
    start: new Date(2023, 4, 5, 10, 0),
    end: new Date(2023, 4, 5, 11, 0),

  },
  {
    title: 'Birthday party',
    start: new Date(2023, 4, 6, 18, 0),
    end: new Date(2023, 4, 6, 20, 0),
  },
  {
    title: 'Client meeting',
    start: new Date(2023, 4, 8, 9, 0),
    end: new Date(2023, 4, 8, 11, 0),
  },
  {
    title: 'Date night',
    start: new Date(2023, 4, 12, 19, 0),
    end: new Date(2023, 4, 12, 22, 0),
  },
  {
    title: 'Parent-teacher conference',
    start: new Date(2023, 4, 15, 15, 0),
    end: new Date(2023, 4, 15, 16, 0),
  },
  {
    title: 'Doctor appointment',
    start: new Date(2023, 4, 16, 11, 0),
    end: new Date(2023, 4, 16, 12, 0),
  },
  {
    title: 'Team building activity',
    start: new Date(2023, 4, 18, 13, 0),
    end: new Date(2023, 4, 18, 17, 0),
  },
  {
    title: 'Sales presentation',
    start: new Date(2023, 4, 22, 10, 0),
    end: new Date(2023, 4, 22, 12, 0),
  },
];

export default seedEvents;